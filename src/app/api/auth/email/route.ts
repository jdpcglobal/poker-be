...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!idToken || typeof idToken !== 'string') {
      throw new AuthError('MISSING_EMAIL_TOKEN', 'idToken is required');
    }

    const { email, firebaseUid } = await verifyFirebaseToken(idToken);

    let user = await User.findOne({
      'authProviders.provider': 'email',
      'authProviders.providerId': firebaseUid,
    });
    ...
    if (user) {
      ...
      user = await User.create({
        ...
        authProviders: [{ provider: 'email', providerId: firebaseUid, email }],
      });
      isNewUser = true;
    }
